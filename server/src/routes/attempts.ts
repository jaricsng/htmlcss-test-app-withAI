...
router.post('/start', requireAuth, requireRole('student'), (req, res) => {
  ...
  // Get existing submissions for this attempt
  const submissions = db.prepare(
    'SELECT * FROM submissions WHERE attempt_id = ?'
  ).all(attempt.id);

  res.json({ attempt, submissions });
});

// Student: save progress (auto-save)
router.put('/:attemptId/questions/:questionId', requireAuth, requireRole('student'), (req, res) => {
  ...
  `).run(
    req.params.attemptId, req.params.questionId,
...
// Student: submit test (triggers grading)
router.post('/:attemptId/submit', requireAuth, requireRole('student'), (req, res) => {
  ...
  const gradeQuestion = db.transaction((question: any) => {
    ...
    if (question.type === 'mcq') {
      const correctIdx = question.mcq_correct_index;
      score = gradeMcq(submission.mcq_answer_index, correctIdx, maxScore);
      gradingResults = [{
...
// Lecturer: view all results for a test
