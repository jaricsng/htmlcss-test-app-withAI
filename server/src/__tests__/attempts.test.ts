...
async function seedPublishedTest(
  ...
  const { body: { id: codeQId } } = await request.post('/api/questions')
    .set(bearer(lecturerToken))
    .send({
      test_id: testId, type: 'code-from-scratch', order_index: 0,
      title: 'Build a nav', description: 'desc', total_points: 10,
    });

  await request.post(`/api/questions/${codeQId}/criteria`)
    .set(bearer(lecturerToken))
  ...
  const { body: { id: mcqQId } } = await request.post('/api/questions')
    .set(bearer(lecturerToken))
    .send({
      test_id: testId, type: 'mcq', order_index: 1, title: 'CSS meaning',
      description: 'desc', total_points: 5,
...
describe('Attempts & grading API', () => {
  ...
  describe('POST /api/attempts/start', () => {
    it('creates a new attempt for a published test', async () => {
      ...
      const res = await request.post('/api/attempts/start')
        .set(bearer(studentToken)).send({ test_id: testId });
    ...
    it('resumes an existing in-progress attempt (same attempt id returned)', async () => {
      const { testId } = await seedPublishedTest(request, lecturerToken);
      const first = await request.post('/api/attempts/start')
        .set(bearer(studentToken)).send({ test_id: testId });
      const firstId = first.body.attempt.id;

      const second = await request.post('/api/attempts/start')
        .set(bearer(studentToken)).send({ test_id: testId });
  ...
  describe('PUT /api/attempts/:id/questions/:qid', () => {
    it('saves HTML and CSS answers', async () => {
      ...
      const { body: { attempt } } = await request.post('/api/attempts/start')
        .set(bearer(studentToken)).send({ test_id: testId });

      const res = await request
        .put(`/api/attempts/${attempt.id}/questions/${codeQId}`)
        .set(bearer(studentToken))
        .send({ html_code: '<nav class="navbar"><a>Home</a></nav>', css_code: 'nav { color: red; }' });
      expect(res.status).toBe(200);
    ...
    it('saves MCQ answer index', async () => {
      ...
      const res = await request
        ...
        .set(bearer(studentToken)).send({ mcq_answer_index: 0 });
      expect(res.status).toBe(200);
    });
    ...
    it("rejects saves for another student's attempt", async () => {
      ...
      const res = await request
        .put(`/api/attempts/${attempt.id}/questions/${codeQId}`)
        .set(bearer(other.token)).send({ html_code: '<p>hacked</p>' });
  ...
  describe('POST /api/attempts/:id/submit', () => {
    it('grades a perfect submission — 15/15', async () => {
      ...
      await request.put(`/api/attempts/${attempt.id}/questions/${codeQId}`)
        .set(bearer(studentToken))
        .send({ html_code: '<nav class="navbar"><a href="#">Home</a></nav>', css_code: '' });
      await request.put(`/api/attempts/${attempt.id}/questions/${mcqQId}`)
        .set(bearer(studentToken)).send({ mcq_answer_index: 0 });

      const res = await request.post(`/api/attempts/${attempt.id}/submit`)
        .set(bearer(studentToken)).send({});
      expect(res.status).toBe(200);
    ...
    it('grades a partial submission — 5/15', async () => {
      ...
      await request.put(`/api/attempts/${attempt.id}/questions/${mcqQId}`)
        .set(bearer(studentToken)).send({ mcq_answer_index: 2 });
    ...
    it('rejects a second submission with 400', async () => {
      ...
      const { body: { attempt } } = await request.post('/api/attempts/start')
        .set(bearer(studentToken)).send({ test_id: testId });
      await request.post(`/api/attempts/${attempt.id}/submit`)
        .set(bearer(studentToken)).send({});
  ...
  describe('GET /api/attempts/:id/results', () => {
    it('returns per-criterion grading results', async () => {
      ...
      const res = await request.get(`/api/attempts/${attempt.id}/results`)
        .set(bearer(studentToken));
      expect(res.status).toBe(200);
  ...
  describe('GET /api/attempts/test/:testId/results', () => {
    it('returns all student attempts with total scores', async () => {
      ...
      const res = await request.get(`/api/attempts/test/${testId}/results`)
        .set(bearer(lecturerToken));
      expect(res.status).toBe(200);
