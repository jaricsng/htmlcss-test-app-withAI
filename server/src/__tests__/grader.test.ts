...
describe('gradeSubmission – DOM checks', () => {
  ...
  it('fails when element is missing', () => {
    const { results, score } = gradeSubmission('<div>no nav here</div>', '', [
      { id: 1, type: 'dom', label: 'Has nav.navbar', selector: 'nav.navbar', points: 5 },
    ]);
    expect(score).toBe(0);
  ...
  it('passes attribute check when value matches', () => {
    const { results, score } = gradeSubmission('<a href="https://example.com">Link</a>', '', [
      {
        id: 1,
        type: 'dom',
        label: 'Link href correct',
        selector: 'a',
        attribute: 'href',
  ...
  it('fails attribute check when value is wrong', () => {
    const { results, score } = gradeSubmission('<a href="/wrong">Link</a>', '', [
      {
        id: 1,
        type: 'dom',
        label: 'Link href correct',
  ...
  it('passes text content check', () => {
    const { results, score } = gradeSubmission('<h1>Hello World</h1>', '', [
      {
        id: 1,
        type: 'dom',
        label: 'H1 contains Hello',
        selector: 'h1',
  ...
  it('handles invalid CSS selector gracefully', () => {
    const { results, score } = gradeSubmission('<div></div>', '', [
  ...
  it('scores partial marks when only some criteria pass', () => {
    const { score, maxScore } = gradeSubmission('<nav class="navbar"></nav>', '', [
      { id: 1, type: 'dom', label: 'Has nav.navbar', selector: 'nav.navbar', points: 5 },
...
describe('gradeSubmission – CSS style checks', () => {
  it('passes when CSS property matches in stylesheet', () => {
    const { results, score } = gradeSubmission('<h1>Title</h1>', 'h1 { color: red; }', [
      {
        id: 1,
        type: 'style',
        label: 'h1 is red',
  ...
  it('normalises colour names when comparing', () => {
    const { results } = gradeSubmission('<p>text</p>', 'p { color: #ff0000; }', [
      {
        id: 1,
        type: 'style',
        label: 'p is red',
  ...
  it('fails when CSS property is not set', () => {
    const { results, score } = gradeSubmission('<h1>Title</h1>', 'h1 { font-size: 24px; }', [
      {
        id: 1,
        type: 'style',
        label: 'h1 is blue',
  ...
  it('fails when CSS value does not match', () => {
    const { results, score } = gradeSubmission('<h1>Title</h1>', 'h1 { color: blue; }', [
      {
        id: 1,
        type: 'style',
        label: 'h1 is red',
  ...
  it('fails gracefully when selector or property missing', () => {
    const { results } = gradeSubmission('<h1>Title</h1>', 'h1 { color: red; }', [
